function getPulicTweet() {
	...
	var publictweet = document.getElementById("publictweet");
		...
		for(var i = tweets.length -1; i >= 0; i--) {
			...
			htmlStr += "<div class='floatDiv'><div><a href='personal.html'><span class='tweetUserID'>" + tweets[i]["follower"] + "</span></a><span class='tweetDate'>" + tweets[i]["date"] + "</span></div>";
			htmlStr += "<div class='tweetMsg'>" + tweets[i]["msg"] + "</div>";
			htmlStr += "</div>"//"<div><img src='images/tweet_" + tweets[i]["seq"] + ".jpg' width='500'></div></div>";
			htmlStr += "</div>";
		}
